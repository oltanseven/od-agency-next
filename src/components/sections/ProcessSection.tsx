...
export default function ProcessSection() {
  ...
  return (
    <section id="surec" className="py-28 sm:py-40 bg-cream-soft">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-[960px] mx-auto">
          {steps.map((step, i) => (
            <div key={step.num} className="relative text-center md:text-left">
              {i < steps.length - 1 && (
                <div className="hidden md:block absolute top-8 right-0 w-6 h-px bg-ink/10 translate-x-3" />
              )}
              <div className="text-[3rem] font-black text-accent/20 leading-none mb-4">{step.num}</div>
              <h3 className="text-[1.1rem] font-bold text-ink mb-2">{step.title}</h3>
              <p className="text-ink/50 text-[0.9rem] leading-[1.7] mb-4">{step.desc}</p>
