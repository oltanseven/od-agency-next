...
export default function SolutionSection() {
  ...
  return (
    <section id="solutions" className="py-28 sm:py-40 bg-[#0f172a] text-white">
          ...
          {cards.map(({ title, desc, tag }) => (
            <div key={title}
              className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-8 sm:p-10
                hover:shadow-lg hover:bg-white/[0.06] transition-all duration-300">
              <h3 className="text-[1.15rem] font-bold text-white mb-3">{title}</h3>
              <p className="text-white/50 text-[0.92rem] leading-[1.7] mb-5">{desc}</p>
